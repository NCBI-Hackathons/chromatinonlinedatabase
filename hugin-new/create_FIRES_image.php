...
function draw_highlight($img, $passthru, $plot_area)//draws highlight - I don't know if functions can be addressed in different files or not
{
...
function draw_fire($img, $passthru, $plot_area)//draws fire regions
{
	list($plot, $y2 ,$x_world1,$x_world2) = $passthru;
    list($x1,$ytest)=$plot->GetDeviceXY($x_world1,$y2);
    list($x2,$ytest)=$plot->GetDeviceXY($x_world2,$y2);
    $color_fg = imagecolorresolve($img, 255,0,0); // red
	imagefilledrectangle($img, $x1, 1, $x2, 100, $color_fg);
}
...
$con=mysqli_connect("thirty-four.its.unc.edu","hic_db_user","ro_hic","hic_db");//sets up database connection
...
if($field_name=="IMR90")//IMR90 hack
...
$result=mysqli_query($con,$sql);//gets query 
$gene_data=array();
if ($result){
...
$plot=plot_chromosome($plot,$gene_data,array(200, 0, 4600),$range,$MidPoint);
$plot->PrintImage();
...
#    $data= array_map(NULL,array_column($data,0),array_column($data,1),array_column($data,2));
    ...
    if($MidPoint>$range[1] && $MidPoint<$range[2]){
        $plot->SetCallback('draw_all', 'draw_line', array($plot, 0,25*4,intval($MidPoint),intval($MidPoint)+5));//draws vertical line
        $plot->DrawGraph();
    }
    foreach($data as $fire_info){
        $plot->SetCallback('draw_all', 'draw_fire', array($plot,$plot_area[1]+25*4,$fire_info[1],$fire_info[2]));//draws fire function
        $plot->DrawGraph();
    }
    
    $plot->SetCallback('draw_all','label_figure',array($plot,40,0,"FIREs",40));//labels graph
    $plot->DrawGraph();
