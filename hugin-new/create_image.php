...
function label_figure($img, $passthru, $plot_area)//text labeling figure
{
...
$hic_data=$_SESSION["hic_data".$field_name];// $_SESSION["hic_data"]; - contains the Hi-C data for a given field name
...
array_push($data,$last);//puts it back

if($range[2]-$last[1]<40000){
    $last[1]=$range[2];//changes end data point
    array_push($data,$last);//attaches the last point back on with the range extended
}else{
    $last=array('',$last[1],0,0,0);//changes end data point
    array_push($data,$last);//adds to data array
...
$plot=new PHPlot_truecolor($WIDTH+$Xpad,$HEIGHT+$Xpad);#each graph area is 800 pixels high- with extra for labeling
