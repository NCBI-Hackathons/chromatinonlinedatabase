...
function draw_highlight($img, $passthru, $plot_area)//draw highlight regions
{
...
function draw_SE($img, $passthru, $plot_area)//draw SE
{
	list($plot, $y2 ,$x_world1,$x_world2) = $passthru;
    list($x1,$ytest)=$plot->GetDeviceXY($x_world1,$y2);
    list($x2,$ytest)=$plot->GetDeviceXY($x_world2,$y2);
...
function draw_TE($img, $passthru, $plot_area)//draw TEs
{
	list($plot, $y2 ,$x_world1,$x_world2) = $passthru;
    list($x1,$ytest)=$plot->GetDeviceXY($x_world1,$y2);
    list($x2,$ytest)=$plot->GetDeviceXY($x_world2,$y2);
    $color_fg = imagecolorresolvealpha($img, 0,125,0,25); // green
	imagefilledrectangle($img, $x1, 0, $x2, $y2, $color_fg);
}
...
function draw_line($img, $passthru, $plot_area)//draws vertical line
{
...
if($MidPoint>$range[1] && $MidPoint<$range[2]){//draw midpoint
...
if($field_name=="IMR90")//IMR90 hack for building mysql
...
$result=mysqli_query($con,$sql);//gets mysql query
...
if ($result){
    ...
    if(strpos($display,"TE")!==false){//if TE is in display TE
