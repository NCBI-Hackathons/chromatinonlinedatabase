...
{
	list($plot, $y2 ,$x_world1,$name) = $passthru;
    list($x1,$ytest)=$plot->GetDeviceXY($x_world1,$y2);    
...
//    imagefilledrectangle($img,$x1,0,$x1,$y2,$color_fg);
    ...
    imageline($img,$x1,0,$x1,$y2,$color_fg);
}
...
//        $chip_data[]=array('',intval($row["startbp"]),$row["H3K27Ac_1"]);
//        $chip_data[]=array('',intval($row["startbp"])+40000,$row["H3K27Ac_1"]);
    }
}
...
#_truecolor
$plot=new PHPlot_truecolor(1200*4,150*4);#each graph area is 100 pixels high- with extra for labeling
...
$plot->SetLegendPosition(1,0,'plot',1,0,60,-420);
...
if(count($data)==2)
    {
        $plot->SetCallback('draw_all','label_figure',array($plot,300,800,"No ChIP-seq data Available",80));
$plot->DrawGraph();
    }
...
$plot->SetLineWidth(2);
$plot->SetDefaultDashedStyle('40-60');
...
$plot->SetDataColors(array('orange','magenta'));
$plot->DrawGraph();
