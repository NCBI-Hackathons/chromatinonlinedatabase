...
#    imageellipse($img,$x,$y,100*$diff,100*$diff,$line_color);
    imagesetthickness($img,5);
...
#    Imagettftext($img,$size,0,floor($x_world),floor($y2),$line_color,'./Intrepid.ttf',$label);
}
...
function plot_maps($plot,$data,$plot_area,$bin_size,$range,$max_pval,$field_name,$MidPoint){
...
# $plot->SetTitle($y);
#$plot->SetXTitle('Bases');
#$plot->SetYTitle('counts','plotleft');
#$plot->SetBackgroundColor('white');
#$plot->SetTransparentColor('white');
#$plot->SetDrawPlotAreaBackground(FALSE);
$plot->DrawGraph();
