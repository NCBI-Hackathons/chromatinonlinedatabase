...
function draw_CTCF($img, $passthru, $plot_area)
{
    ...
    $color_border = imagecolorresolve($img,0,0,255);//black
...
#_truecolor
$plot=new PHPlot_truecolor(1200*4,100);#each graph area is 100 pixels high- with extra for labeling
$plot->SetPrintImage(FALSE);#disables auto-output
//$field_name);
#$plot=plot_chromosome($plot,$gene_data,array(50, 0, 1200),$range);
$plot_area=array(200,0,4*1150);
...
$sql="select max(peaks) from CTCF_".$field_name." WHERE chr='".$chr."'";
...
if ($result){
    while($row=mysqli_fetch_array($result)){
        ...
        if($row["endbp"]>$range[2])
            $row["endbp"]=$range[2];
        $CTCF_data[]=array('',intval($row["startbp"]),intval($row["endbp"]),floatval($row["peaks"]));
    }

    
    foreach($CTCF_data as $gene_info){
